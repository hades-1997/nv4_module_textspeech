...
$array_voice = json_decode(file_get_contents('https://viettelgroup.ai/voice/api/tts/v1/rest/voices'), true);

$array_speed = array( 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75);
//var_dump($array_voice);die;
...
//$array_config['voice'] = "'".$array_config['voice']."'";
...
foreach ($array_voice as $key => $title){
	//var_dump($key);die;
	 $xtpl->assign('VOICE', [
        'key' =>$title["code"],
        'title' => $title["name"].' - '.$title["description"],
        'selected' => $title["code"] == $array_config['voice'] ? ' selected="selected"' : ''
    ]);
	 $xtpl->parse('main.loop');
}
foreach ($array_speed as $key => $title){
	//var_dump($key);die;
	 $xtpl->assign('SPEED', [
        'key' =>$key,
        'title' => $title,
        'selected' => $key == $array_config['speed'] ? ' selected="selected"' : ''
    ]);
